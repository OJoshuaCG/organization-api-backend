...
class UserController
{
    ...
    public function indexByOrganization(Request $request, int $organizationId): JsonResponse
    {
        $filters = $request->only([
            'username', 'email', 'first_name', 'last_name',
            'is_active', 'user_role_id', 'company_id'
        ]);
        $perPage = $request->input('per_page', 15);

        $users = $this->userService->getByOrganizationWithFilters($organizationId, $filters, $perPage);
    ...
    public function indexByCompany(Request $request, int $companyId): JsonResponse
    {
        $filters = $request->only([
            'username', 'email', 'first_name', 'last_name',
            'is_active', 'user_role_id'
        ]);
        $perPage = $request->input('per_page', 15);

        $users = $this->userService->getByCompanyWithFilters($companyId, $filters, $perPage);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $user = $this->userService->createForTenant($validated);
